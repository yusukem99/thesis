...
卒業制作では、Webサーバやデータベースなどのソフトウェアを動かす環境を用意し、その上でプログラムを開発します。

この回では、**実行環境の用意**（どこで動かすか）から、開発に使う**道具**（サーバへの接続・エディタ・Webサーバ）、そして完成したものを**インターネットに公開する**方法までを扱います。

## 実行環境を用意する

教室のWindowsにインストールできるソフトウェアは制限されているため、必要に応じて仮想環境やクラウドを利用します。以下に、主な方法を紹介します。
