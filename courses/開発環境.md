...
この回では、まず開発に使う**道具**（エディタ・バージョン管理・AI）をそろえ、次に**実行環境の用意**（どこで動かすか）、サーバへの接続とWebサーバ、そして完成したものを**インターネットに公開する**方法までを、まとめて扱います。

この回で学ぶ git は、**制作するコード（プログラム）の管理**に使います。計画書・スケジュール・週次報告書などの書類は、Google Classroom で配布・提出します。

```mermaid
flowchart LR
    A["① 道具をそろえる<br/>VS Code・Git・AI"] --> B["② 実行環境を用意する<br/>Colab・VPS・Docker"]
    B --> C["③ 接続して動かす<br/>SSH・Apache"]
    C --> D["④ インターネットに公開する<br/>トンネル・ホスティング"]
```
...
まずは自分の端末に Visual Studio Code をインストールし、以降で説明するバージョン管理（Git）やAIによるコーディング補助も、このエディタの中から使えるようにしていきます。
...
```mermaid
flowchart TD
    E["report.php を書き直したい"] --> Q{"どう管理するか"}
    Q -- "コピーして残す" --> A["report_new.php<br/>report_最終.php<br/>report_最終2_これ.php<br/>→ どれが最新か分からない"]
    Q -- "Git で記録する" --> B["ファイルは report.php の1つだけ<br/>変更の履歴が別に積み上がる<br/>→ いつでも過去の状態に戻せる"]
```
...
Gitのコマンドは、ばらばらに覚えるとなかなか身につきません。**ファイルが置かれる4つの場所**と、**その間を移動させるコマンド**として捉えると、全体の見通しがよくなります。

```mermaid
flowchart LR
    W["作業ディレクトリ<br/>いま編集しているファイル"] -- "git add" --> S["ステージングエリア<br/>次に記録する候補"]
    S -- "git commit" --> L["ローカルリポジトリ<br/>自分の端末に残る履歴"]
    L -- "git push" --> R["リモートリポジトリ<br/>GitHub上の履歴"]
    R -- "git pull" --> W
    R -. "git clone（最初の1回）" .-> W
```
...
```mermaid
flowchart LR
    C1["1回目<br/>はじめのコミット"] --> C2["2回目<br/>検索フォームを追加"]
    C2 --> C3["3回目<br/>予算での絞り込み機能を追加"]
    C3 --> N["いまの作業<br/>まだコミットしていない"]
    C3 -. "動かなくなったらここまで戻せる" .-> C2
```

これが「こまめにコミットしなさい」と言われる理由です。コミットの間隔が長いほど、戻れる地点が少なくなり、戻したときに失う作業も大きくなります。
...
`git status` は、1つ1つのファイルが次のどの状態にあるかを教えてくれます。コマンドを実行する前後で状態がどう変わるかを意識すると、操作の意味が分かりやすくなります。

```mermaid
flowchart TD
    F["ファイルを編集・追加した"] --> U["変更あり（未ステージ）<br/>git status で赤く表示される"]
    U -- "git add" --> G["ステージ済み<br/>git status で緑に表示される"]
    G -- "git commit" --> D["コミット済み<br/>履歴に記録された"]
    D -- "git push" --> P["GitHubと同じ状態<br/>nothing to commit と表示される"]
```

思ったとおりに操作できているか迷ったときは、まず `git status` を実行して、いまどの状態にいるのかを確かめましょう。
...
```mermaid
flowchart TD
    R["GitHubの最新状態<br/>search.php"] --> A["Aさんが search.php の<br/>同じ行を編集"]
    R --> B["Bさんも search.php の<br/>同じ行を編集"]
    A -- "git push（先に成功）" --> R2["GitHubが更新される"]
    B -- "git push" --> X["拒否される<br/>先に git pull が必要"]
    X --> Y["git pull するとコンフリクト<br/>どちらを残すか手で決める"]
```
...
この授業で git を使うのは、**制作するコード（プログラム）の管理**です。**こまめにコミット・プッシュ**しておけば、コミットの履歴がそのまま作業の記録になり、動かなくなったときも前の状態に戻せます。

なお、計画書・スケジュールは Google Classroom（Google ドキュメント）、週次報告書は Google フォームで提出します。書類の提出に git は使いません。

```mermaid
flowchart LR
    C["制作するコード<br/>プログラム・設定ファイル"] --> G["Git / GitHub<br/>こまめにコミット・プッシュ"]
    P["計画書・スケジュール"] --> GC["Google Classroom<br/>Googleドキュメント"]
    W["週次報告書"] --> GF["Googleフォーム"]
```
...
```mermaid
flowchart LR
    A["① 具体的に頼む<br/>目的・言語・条件を伝える"] --> B["② 生成されたコードを読む<br/>意味を理解する"]
    B --> C["③ 動かして確かめる<br/>テストする"]
    C --> D["④ git でコミットする"]
    D --> A
    B -. "分からない・危ないと感じたら<br/>質問して直してもらう" .-> A
```
...
```mermaid
flowchart TD
    S["何を作るか決まった"] --> P{"Python や AI が中心か"}
    P -- はい --> C["Colab + Gradio<br/>試作とデモに向く"]
    P -- いいえ --> W{"常に動かし続けるか"}
    W -- いいえ --> C
    W -- はい --> V["VPS・クラウド<br/>Apache などを立てる"]
    V --> D["Docker<br/>環境をそろえたいとき"]
```

### 手軽に始める：Colab + Gradio + cloudflared

Google Colab を使えば、環境構築をほとんどせずに、開発から公開までをブラウザだけで完結できます。Python や AI を使った制作に向いた、手軽な開発スタイルの一つです。

```mermaid
flowchart LR
    N["Colabのノートブック<br/>ブラウザで開くだけ"] --> P["Pythonのプログラム<br/>Googleのサーバ上で動く"]
    P --> G["Gradio<br/>入力・出力の画面を作る"]
    G --> T["cloudflared<br/>公開URLを割り当てる"]
    T --> U["外部の利用者<br/>スマートフォン・友人のPC"]
```

Colab 上で Gradio を使って操作画面を作り、cloudflared で公開 URL を割り当てれば、VPS や SSH を用意しなくても「作る → 公開する」まで進められます。まずはこの方法で試作し、常に動かし続ける本番環境が必要になったら、このあとで紹介するサーバ（VPS）を使う方法に進むとよいでしょう。

使い方は、Colab のノートブックにまとめてあります。Gradio の入力・出力に使える部品の一覧、`http.server` で自分で HTTP サーバを立てる方法、公開 URL の発行方法が載っています。手元で動かしながら確認してください。

**▶ [Colab で開く：Gradio・HTTPサーバ・cloudflared](https://colab.research.google.com/github/yusukem99/thesis/blob/main/notebooks/gradio-cloudflared.ipynb)**
...
```mermaid
flowchart TD
    H["ホストOS<br/>自分のPCやVPSのLinux"] --> E["Dockerエンジン"]
    E --> C1["webコンテナ<br/>Apache + PHP"]
    E --> C2["dbコンテナ<br/>PostgreSQL"]
    E --> C3["必要なら追加のコンテナ"]
```
...
```mermaid
flowchart LR
    B[ブラウザ] --> W["web コンテナ<br/>Apache :80"]
    W --> D["db コンテナ<br/>PostgreSQL"]
```
...
`php:8.3-apache` は、Apache に PHP を組み込んだ公式イメージです。Apache だけが必要な場合は `httpd:2.4` を使います。
...
```mermaid
flowchart LR
    V["自分のPCの ./web<br/>VS Codeで編集する"] -- "volumes でマウント<br/>中身は同じもの" --> C["webコンテナの /var/www/html<br/>Apacheが配信する"]
    C --> B["ブラウザに反映される"]
```
...
```mermaid
flowchart LR
    P["手元のPC<br/>ターミナル / VS Code"] -- "SSH（暗号化された通信）" --> S["VPS・クラウド上のサーバ<br/>Ubuntu など"]
    S --> F["ファイルの編集<br/>コマンドの実行<br/>Apache・DBの操作"]
```

### 公開鍵認証のしくみ

SSHの接続では、パスワードの代わりに**鍵のペア**を使う「公開鍵認証」が広く使われています。**公開鍵**は接続先のサーバに登録し、**秘密鍵**は自分の端末だけに置きます。秘密鍵そのものはネットワークに流れないため、盗聴されても悪用されません。

```mermaid
flowchart LR
    K["手元のPC<br/>秘密鍵 mykey.pem"] -- "① 接続を要求" --> S["サーバ<br/>公開鍵を登録済み"]
    S -- "② ランダムなデータを送る" --> K
    K -- "③ 秘密鍵で署名して返す" --> S
    S -- "④ 公開鍵で検証できたらログインを許可" --> K
```

秘密鍵は、パスワードと同じか、それ以上に大切なものです。GitHub などにアップロードしたり、他人に渡したりしてはいけません。
...
Windowsの`PowerShell`や、macOS・Linuxの`ターミナル`などのコンソールからssh接続を行うことができます。
...
```mermaid
flowchart LR
    B["ブラウザ"] -- "http://サーバのIP/" --> A["Apache<br/>ポート80で待ち受け"]
    A -- "ファイルを探す" --> D["DocumentRoot<br/>/var/www/html"]
    D -- "index.html" --> A
    A -- "HTMLを返す" --> B
```
...
```mermaid
flowchart LR
    A["sites-available/<br/>設定ファイルを置く"] -- "sudo a2ensite" --> B["sites-enabled/<br/>有効になった設定"]
    B -- "sudo systemctl reload apache2" --> C["Apacheが新しい設定で動く"]
```
...
```mermaid
flowchart TD
    Q{"いつ見せるか"} -- "発表やテストのときだけ" --> T["トンネル<br/>cloudflared / ngrok"]
    Q -- "24時間動かし続ける" --> H{"何を公開するか"}
    H -- "HTML・CSS・JS だけ" --> S["GitHub Pages<br/>Cloudflare Pages など"]
    H -- "AIのデモ" --> F["Hugging Face Spaces"]
    H -- "サーバ側の処理がある" --> V["VPS・クラウド<br/>Apache を立てる"]
```
...
```mermaid
flowchart LR
    U["外部の利用者<br/>スマートフォン・友人のPC"] --> C["公開URL<br/>https://～.trycloudflare.com"]
    C --> T["トンネル<br/>cloudflared / ngrok"]
    T --> L["自分の環境で動くアプリ<br/>localhost:8000"]
```
