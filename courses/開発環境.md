...
Gitは**自分の端末で動くソフトウェア**、GitHubは**インターネット上のサービス**です。名前が似ていますが、別のものです。

```mermaid
flowchart LR
    subgraph PC["自分の端末"]
        F["プロジェクトのファイル"] --> G["Git<br/>履歴を記録するソフトウェア"]
        G --> H[".git<br/>記録された変更履歴"]
    end
    subgraph NET["インターネット"]
        GH["GitHub<br/>リポジトリを預かるサービス"]
    end
    H -- "git push" --> GH
    GH -- "git clone・git pull" --> H
    GH --> M["チームのメンバー"]
    GH --> W["ブラウザで閲覧<br/>公開・Issue"]
```
...
```mermaid
flowchart LR
    T["VS Codeのターミナル"] -- "git コマンド" --> L["変更履歴の記録<br/>add・commit・push"]
    T -- "gh コマンド" --> R["GitHub側の操作<br/>リポジトリ作成・Issue・確認"]
    L --> G["GitHub"]
    R --> G
```
