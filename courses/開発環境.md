...
```mermaid
stateDiagram-v2
    [*] --> Modified: ファイルを編集・追加した
    Modified --> Staged: git add
    Staged --> Committed: git commit
    Committed --> Pushed: git push
    Pushed --> Modified: また編集する

    Modified: 変更あり（未ステージ）
    Staged: ステージ済み
    Committed: コミット済み（自分の端末だけ）
    Pushed: GitHubと同じ状態
```
...
sequenceDiagram
    ...
    A->>G: git pull
    B->>G: git pull
    Note over A,B: 2人とも search.php の同じ行を編集してしまった
    A->>G: git push（成功）
    B->>G: git push
    G-->>B: 拒否される（先に git pull が必要）
    B->>G: git pull
    Note over B: コンフリクト発生<br/>どちらを残すか手で決める
    B->>G: 解決してから git push
```
...
!!! note "データベースの中身を確認するには"
    `docker exec -it seisaku-db psql -U postgres` を実行すると、`db` コンテナの PostgreSQL に接続できます。

!!! tip "環境をチームで共有する"
    `docker-compose.yaml` を、プロジェクトのファイルと一緒に git で管理しておきましょう。ほかの端末でも `docker compose up` だけで同じ環境を再現できます（各端末に Docker をインストールしておく必要はあります）。
...
sequenceDiagram
    autonumber
    participant P as 手元のPC（秘密鍵をもつ）
    participant S as サーバ（公開鍵を登録済み）
    P->>S: 接続を要求する
    S->>P: ランダムなデータを送る
    P->>S: 秘密鍵で署名して返す
    S->>S: 登録済みの公開鍵で署名を検証する
    S-->>P: 一致すればログインを許可する
```
...
sequenceDiagram
    ...
    participant A as Apache（ポート80で待ち受け）
    participant D as DocumentRoot（/var/www/html）
    B->>A: http://サーバのIP/ をリクエスト
    A->>D: index.html を探す
    D-->>A: ファイルの中身
    A-->>B: HTMLを返す
```
