...
この回では、まず開発に使う道具（エディタ・バージョン管理・AI）をそろえ、次に実行環境の用意（どこで動かすか）、サーバへの接続とWebサーバ、そして完成したものをインターネットに公開する方法までを、まとめて扱います。

この回で学ぶ git は、制作するコード（プログラム）の管理に使います。計画書・スケジュール・週次報告書などの書類は、Google Classroom で配布・提出します。
...
- セキュリティを自分でレビューする。AIは、SQLインジェクションを許すコードや、パスワードやAPIキーの直書きを生成することがあります。生成されたコードの安全性は、自分の目で確認してください。
- 過剰な防御は削る。AIは頼まなくても、入力検証や例外処理を厚く付けてきます。妥当なこともありますが、必要のない場所にまで防御が入り、何をしているか追えない量に膨らみがちです。説明できない処理は残さないこと。理解して残すか、外すかのどちらかにします。
...
[Colabでノートブックを開く](https://colab.research.google.com/github/yusukem99/thesis/blob/main/notebooks/gradio-cloudflared.ipynb)
